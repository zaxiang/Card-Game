...
// }
export function areCompatible(card: Card, lastCardPlayed: Card, wildCard: typeof RANKS[number]) {
  return card.rank === wildCard || lastCardPlayed.rank === wildCard || card.rank === lastCardPlayed.rank || card.suit === lastCardPlayed.suit
}
 ...
 */
 export function createEmptyGame(playerNames: string[], numberOfDecks = 5, rankLimit = Infinity, suitLimit = Infinity, wildCard:string): GameState {
  const cardsById: Record<CardId, Card> = {}
  let cardId = 0

  for (let i = 0; i < numberOfDecks; i++) {
    for (const suit of SUITS.slice(0, suitLimit)) {
 ...
 */
export function doAction(state: GameState, action: Action, wildCard: typeof RANKS[number]): Card[] {
  ...
  if (state.phase === "initial-card-dealing") {
    ...
    if (!areCompatible(lastPlayedCard, card, wildCard)) {
      return []
    }
